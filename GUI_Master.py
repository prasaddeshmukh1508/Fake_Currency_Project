...
#
# lbl = tk.Label(root, text="Fake Currency  Detection System", font=('times', 25,' bold '), height=1, width=70,bg="Black",fg="white")
# lbl.place(x=0, y=20)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=350, bd=5, font=('times', 14, ' bold '),bg="#C1CDCD")
...
#    global fn
    ...
    if fn!="":
        ...
        model = load_model('curency_model.h5')
...
def update_label(str_T):
    #clear_img()
    result_label = tk.Label(root, text=str_T, width=60, font=("bold", 20), bg='skyblue', fg='black')
...
def test_model():
    ...
    if fn!="":
        ...
        X1="Selected Note is {0}".format(X)
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/Ashvin/Cavity Detection/Cavity Detection/Dataset', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
